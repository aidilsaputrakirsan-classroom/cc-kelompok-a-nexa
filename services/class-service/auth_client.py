...
async def resolve_users_from_auth_service(user_ids: list[int], authorization: str) -> list[dict]:
    """Batch resolve list user_ids ke user info dari auth-service."""
    if not user_ids:
        return []
    headers = {"Authorization": authorization}
    try:
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{AUTH_SERVICE_URL}/users/resolve",
        ...
        logger.error(f"Failed to resolve users from auth-service: {e}")
    return [{"id": uid, "name": f"User {uid}", "email": "", "role": "mahasiswa", "is_active": True, "created_at": "2024-04-19T10:30:00+00:00"} for uid in user_ids]