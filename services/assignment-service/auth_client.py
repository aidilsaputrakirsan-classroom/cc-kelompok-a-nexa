...
CLASS_SERVICE_URL = os.getenv("CLASS_SERVICE_URL", "http://class-service:8003")

async def verify_instructor_in_class_service(class_id: int, instructor_id: int) -> bool:
    """Panggil Class Service untuk verifikasi apakah instructor_id adalah pengajar kelas."""
    try:
        async with httpx.AsyncClient() as client:
            url = f"{CLASS_SERVICE_URL}/classes/{class_id}/verify-instructor"
            response = await client.get(
                url,
            ...
            if response.status_code == 200:
                return response.json().get("is_instructor", False)
    except Exception as e:
        logger.error(f"Failed to verify instructor in class-service: {e}")
    return False


async def verify_enrollment_in_class_service(class_id: int, student_id: int) -> bool:
    """Panggil Class Service untuk verifikasi apakah student_id terdaftar di kelas."""
    try:
        async with httpx.AsyncClient() as client:
            url = f"{CLASS_SERVICE_URL}/classes/{class_id}/verify-enrollment"
            response = await client.get(
                url,
        ...
        logger.error(f"Failed to verify enrollment in class-service: {e}")
    return False